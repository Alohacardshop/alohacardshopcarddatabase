...
-- Create materialized view for pricing statistics (using correct column names)
CREATE MATERIALIZED VIEW IF NOT EXISTS public.pricing_stats_mv AS
SELECT 
  COUNT(*) FILTER (WHERE started_at >= CURRENT_DATE) as jobs_today,
  ROUND(
    (COUNT(*) FILTER (WHERE status = 'completed' AND started_at >= CURRENT_DATE - INTERVAL '7 days') * 100.0) / 
    NULLIF(COUNT(*) FILTER (WHERE started_at >= CURRENT_DATE - INTERVAL '7 days'), 0),
    2
  ) as success_rate,
  ROUND(
    AVG(EXTRACT(EPOCH FROM (finished_at - started_at)) / 60) FILTER (WHERE status = 'completed' AND started_at >= CURRENT_DATE - INTERVAL '7 days'),
    1
  ) as avg_duration_minutes,
  COALESCE(SUM(variants_updated) FILTER (WHERE started_at >= CURRENT_DATE), 0) as variants_processed_today
