...
class JustTCGClient {
  ...
  private updateMetrics(responseTime: number, success: boolean): void {
    ...
    // Calculate cards per second (rough estimate based on successful requests)
    const elapsedSeconds = (Date.now() - this.metrics.startTime) / 1000;
    this.metrics.cardsPerSecond = this.metrics.successfulRequests * 100 / elapsedSeconds; // Assuming 100 items per request
  }
  ...
  async searchCards(query: string, gameSlug?: string, limit = 50): Promise<JustTCGCard[]> {
    const justTCGSlug = gameSlug ? this.mapGameSlug(gameSlug) : undefined;
    console.log(`Searching cards: ${query}${justTCGSlug ? ` in ${justTCGSlug}` : ''}`);
    ...
    if (justTCGSlug) {
      params.append('game', justTCGSlug);
    }
